...
logging.basicConfig(filename=log_path, level=print)
...
def synthesize_image(texture_path):
    # Model hyperparams
    EPOCHS = 1000
    LEARNING_RATE = .02
    NORM_TERM = 6.
    ...
    with tf.Session() as sess:
        ...
        if not os.path.isdir(out_dir):
            os.makedirs(out_dir)
        return utils.render_img(sess, noise)
...
syn_img = synthesize_image(texture_path)
save = True

if save:
    toimage(syn_img).save(out_path)
else:
    toimage(syn_img).show()
